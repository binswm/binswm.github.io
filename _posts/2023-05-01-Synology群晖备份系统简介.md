...
### 集中备份 Windows PC、服务器、虚拟机（Active Backup for Business (ABB)套件）
...
持续备份模式将自动备份数据，新用户无需执行其他操作即可让数据得到备份。您可从数据恢复入口轻松查找、还原或批量下载所需的项目。
...
### 实时文件同步（Synology Drive ShareSync）
